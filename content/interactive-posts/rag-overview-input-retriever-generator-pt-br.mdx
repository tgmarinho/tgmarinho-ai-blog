...
description: "Um overview denso sobre RAG (Retrieval-Augmented Generation): o porquê da arquitetura existir, o pipeline de ingestão offline, o fluxo de consulta online, embeddings, similaridade do cosseno, HNSW, chunking e por que tudo isso reduz alucinação."
date: "2026-05-22"
published: true
...
> 🧪 **Versão interativa:** este post tem **animações em Canvas** embutidas — RAG flow, cosseno, HNSW e chunking. Brinca com elas no meio do texto. Ou abra o [lab em tela cheia](/lab/rag-overview).

LLMs escrevem bem. Mas quando você pergunta sobre o seu PDF interno, o relatório de Q3 da sua empresa ou algo que aconteceu depois do treinamento, eles fazem uma de duas coisas:
...
Esse post é um overview denso do RAG do começo ao fim — a motivação, a arquitetura mínima, o pipeline de ingestão offline, o fluxo de consulta online, e os conceitos que sustentam tudo (embeddings, cosseno, HNSW, chunking).
...
- **Sabe só o que viu no treino.** Não tem acesso a documentos privados, dados recentes, base interna da sua empresa. O cutoff é uma parede.
- **Alucina quando perguntado fora do treino.** Em vez de dizer "não sei", completa com algo plausível — porque o modelo é otimizado pra continuar texto, não pra reconhecer ignorância.
...
```text
       ┌─────────────────┐                ┌─────────────────┐
       │   LLM puro      │                │  Retrieval puro │
       ├─────────────────┤                ├─────────────────┤
       │ + texto fluido  │                │ + acha conteúdo │
       │ − alucinação    │                │ − sem síntese   │
       │ − sem dado novo │                │ − resposta crua │
       └────────┬────────┘                └────────┬────────┘
                │                                  │
                └───────────────┬──────────────────┘
                                ▼
                          ...
                          └────┬────┘
                               ▼
              respostas precisas + contextualizadas
              + fonte rastreável + menos alucinação
```
...
```text
Input Query ──▶ Retriever ──▶ Generator ──▶ Resposta
   (pergunta)   (busca no    (LLM com
                 corpus)      contexto)
```
...
```text
╔══════════════════════════════════════════════════════════════════╗
║  INGESTÃO · offline · roda 1× (ou quando o corpus muda)          ║
╠══════════════════════════════════════════════════════════════════╣
║                                                                  ║
║  Fontes   →  Text     →  Splitter  →  Embedding  →  Vector DB    ║
║  (PDF,       Loader      (chunks       Model         (HNSW)      ║
║   HTML,      (→ texto)   + meta)       (→ vetor)                 ║
║   DOCX,                                                          ║
║   vídeo,                                                         ║
║   imagem)                                                        ║
║                                                                  ║
...
╔══════════════════════════════════════════════════════════════════╗
║  CONSULTA · online · roda a cada pergunta                        ║
╠══════════════════════════════════════════════════════════════════╣
║                                                                  ║
║  Usuário → query → Embedding → Similarity → top-K → Prompt → LLM ║
║                    da query    Search        chunks    montado   ║
║                                (cosseno)               (system +  ║
║                                                         query +   ║
║                                                         contexto) ║
║                                                                  ║
...
Antes de mergulhar nas etapas em detalhe, brinca com o pipeline ao vivo: clica em **Disparar pergunta** e segue as partículas. As outras abas — **Cosine 2D**, **HNSW** e **Chunking** — aprofundam conceitos que retomamos em texto adiante.
...
## Parte 1 — Ingestão (offline)

### 1. Text Loader · "tudo vira texto"
...
### 3. Embedding Model · cada chunk vira vetor
...
```text
chunk: "Daniel Romero é o fundador da Dev Eficiente."
                          │
                          ▼
                ┌──────────────────┐
                │ Embedding Model  │
...
```text
{
  ...
  texto_original: "Daniel Romero é ...",
...
### B. A pergunta vira string

Texto literal da query — exatamente o que o usuário escreveu, sem transformação semântica:

```text
"Quem é Daniel Romero?"
```

(Existem variações avançadas fora do RAG mínimo: *query rewriting*, **HyDE** (gerar uma resposta hipotética antes de embeddar), **multi-query** (gerar variações da pergunta e buscar com todas). Aqui é só passar adiante.)
...
```text
"Quem é Daniel Romero?" ──▶ Embedding Model ──▶ [0.27, -0.06, 0.27, ...]
                            (mesmo da ingestão)     vetor de 1536 dims
```

### D. Similarity Search no Vector DB

O vector DB compara o vetor da query com os vetores indexados via **similaridade do cosseno** (ou produto interno, ou distância euclidiana) e devolve os **top-K** chunks com maior score.

#### Cosseno — a fórmula e a intuição
...
| Técnica         | O que faz                                                                      |
| --------------- | ------------------------------------------------------------------------------ |
| **Busca híbrida** | Combina cosseno (semântico) com BM25 (keyword). Cobre os pontos cegos de cada um. |
| **Filtros por metadados** | `WHERE autor = "Romero" AND ano >= 2024`. Reduz o espaço de busca antes do cosseno. |
| **Reranking**   | Recupera top-50 com cosseno (barato), passa por um cross-encoder (caro, mais preciso), fica com top-3. |
...
Pergunta: Quem é Daniel Romero?

Contexto:
  [Chunk 1] Daniel Romero é o fundador da Dev Eficiente...
  [Chunk 2] ...trabalhou anos com Java e DDD antes de...
  [Chunk 3] ...lançou o curso de Engenharia de IA em...
```
...
C. Embedding      mesma modelo da ingestão → vetor query
D. Similarity     cosseno no HNSW       → top-K chunks
E. Prompt         system + query + chunks → prompt final
F. LLM            prompt                 → resposta gerada
G. Entrega        resposta + citações    → usuário
```

Esse é o RAG **mínimo viável**. Tudo o que se discute em RAG avançado — chunking semântico, multi-query, HyDE, reranking, agentes — é refinamento de uma dessas oito etapas, não substituição do esqueleto.
