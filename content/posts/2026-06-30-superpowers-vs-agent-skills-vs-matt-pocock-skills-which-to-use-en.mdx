...
I published the benchmark and source files in the [comparison repository](https://github.com/tgmarinho/agent-super-powers-matt-pocock-skills-comparasions).
...
I used the snapshots recorded in that repository. In the source review, Superpowers had 14 `SKILL.md` files; Agent Skills had 24; Matt Pocock's Skills had 25 active skills outside `deprecated` and `in-progress` folders (36 if you count everything).
...
The benchmark is secondary evidence. It shows how those instructions appear in one controlled React task, but it does not prove general performance across all repositories, models, or prompts.
...
## How did I run the benchmark?

To compare all three with the same task, I created two exercises. The first was a real implementation benchmark. The second was a smaller tabletop exercise to compare the PRD, SPEC, and reasoning shape each skill set tends to produce.

The main benchmark is in [`benchmarks/skill-benchmark/`](https://github.com/tgmarinho/agent-super-powers-matt-pocock-skills-comparasions/tree/main/benchmarks/skill-benchmark). The task was to build the same React app three times, once per competitor, each isolated in its own directory:

- `benchmarks/skill-benchmark/implementations/addyosmani/`
- `benchmarks/skill-benchmark/implementations/superpowers/`
- `benchmarks/skill-benchmark/implementations/mattpocock/`
...
The second exercise is in [`benchmarks/skill-framework-benchmark/`](https://github.com/tgmarinho/agent-super-powers-matt-pocock-skills-comparasions/tree/main/benchmarks/skill-framework-benchmark). It was smaller: a React component called `TaskRadar`, with a PRD, SPEC, and implementation. The goal was not to measure a full UI, but to observe the kind of output each framework encourages.
...
- `benchmarks/skill-framework-benchmark/shared-task.md`, with the common prompt.
- `benchmarks/skill-framework-benchmark/superpowers/`, with the Superpowers-style PRD, SPEC, and implementation.
- `benchmarks/skill-framework-benchmark/agent-skills/`, with the Agent Skills-style PRD, SPEC, and implementation.
- `benchmarks/skill-framework-benchmark/mattpocock/`, with the Matt Pocock-style PRD, SPEC, and implementation.
- `benchmarks/skill-framework-benchmark/report.md`, with the comparative scoring.

Important scope detail: the React code-quality section below refers only to the Todo Decision Board in `benchmarks/skill-benchmark/`, not to the smaller `TaskRadar` exercise. In `TaskRadar`, the Agent Skills implementation uses `useReducer`, while the Matt Pocock implementation uses `useState` with small domain helpers.

For that reason, the `TaskRadar` exercise does not support the `useReducer` or `localStorage` claims in the React code analysis. It is only used to compare PRD, SPEC, and process shape.
...
## What came out of the benchmark?
...
This article was produced with AI. The comparison repository and benchmarks were run by agents using only the skills mentioned in this article, without extra React, React Patterns, Next.js, or Vercel skills acting as judges during the original run. The React code-quality section refers to `benchmarks/skill-benchmark/`. The smaller `benchmarks/skill-framework-benchmark/` exercise is used only for PRD, SPEC, and process-shape comparison. The author's review focused on text coherence, clarity, cited evidence, and general consistency.
