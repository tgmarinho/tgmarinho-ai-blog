...
The minimum validation for the implementations was strict TypeScript through `bunx tsc`. The original command from the brief hit `TS5112` because the root `tsconfig.json` was present while explicit files were passed. The equivalent validation was rerun with `--ignoreConfig` and passed for the implementations.
