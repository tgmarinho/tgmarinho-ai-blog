...
Publiquei o benchmark e os arquivos de referência no [repositório de comparação](https://github.com/tgmarinho/agent-super-powers-matt-pocock-skills-comparasions).
...
Usei os snapshots registrados nesse repositório. Na revisão das fontes, Superpowers tinha 14 arquivos `SKILL.md`; Agent Skills tinha 24; Matt Pocock Skills tinha 25 skills ativas fora das pastas `deprecated` e `in-progress` (36 se contar tudo).
...
O benchmark é evidência secundária. Ele mostra como essas instruções aparecem em uma tarefa React controlada, mas não prova desempenho geral em todos os repositórios, modelos ou prompts.
...
## Como executei o benchmark?

Para comparar os três com a mesma tarefa, criei dois exercícios. O primeiro foi um benchmark com implementação real. O segundo foi um exercício menor, de mesa, para comparar o formato de PRD, SPEC e raciocínio que cada conjunto de skills tende a produzir.

O benchmark principal está em [`benchmarks/skill-benchmark/`](https://github.com/tgmarinho/agent-super-powers-matt-pocock-skills-comparasions/tree/main/benchmarks/skill-benchmark). A tarefa foi construir o mesmo app React três vezes, uma vez por competidor, cada um isolado no seu diretório:
...
O segundo exercício está em [`benchmarks/skill-framework-benchmark/`](https://github.com/tgmarinho/agent-super-powers-matt-pocock-skills-comparasions/tree/main/benchmarks/skill-framework-benchmark). Ele foi menor: um componente React chamado `TaskRadar`, com PRD, SPEC e implementação. O objetivo não era medir UI completa, mas observar o tipo de saída que cada framework incentiva.
...
- `benchmarks/skill-framework-benchmark/shared-task.md`, com o prompt comum.
- `benchmarks/skill-framework-benchmark/superpowers/`, com PRD, SPEC e implementação no estilo Superpowers.
- `benchmarks/skill-framework-benchmark/agent-skills/`, com PRD, SPEC e implementação no estilo Agent Skills.
- `benchmarks/skill-framework-benchmark/mattpocock/`, com PRD, SPEC e implementação no estilo Matt Pocock.
- `benchmarks/skill-framework-benchmark/report.md`, com a pontuação comparativa.

Escopo importante: a seção de qualidade de código React abaixo se refere apenas ao Todo Decision Board em `benchmarks/skill-benchmark/`, não ao exercício menor `TaskRadar`. No `TaskRadar`, a implementação de Agent Skills usa `useReducer`, enquanto a implementação de Matt Pocock usa `useState` com pequenos helpers de domínio.

Por isso, o exercício `TaskRadar` não sustenta as afirmações sobre `useReducer` ou `localStorage` na análise de código React. Ele serve apenas para comparar PRD, SPEC e formato de processo.
