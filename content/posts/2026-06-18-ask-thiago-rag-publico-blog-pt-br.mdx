...
A pergunta inicial era onde uma interface agentic faria sentido ao redor deste projeto sem transformar o site em um laboratório solto. O recorte foi pragmático: manter Next.js e Velite como runtime público, usar apenas conteúdo publicado e criar uma busca com resposta citada.
...
5. priorizar documentos no mesmo idioma da página;
6. retornar até seis fontes com excerpt.

Esse ajuste também resolveu perguntas reais como "o que ele sabe sobre react?", "Thiago sabe React Native?" e "Thiago sabe TypeScript?". A resposta precisa cruzar currículo, projetos e posts. React Native aparece no currículo, na experiência com SwitchCare e em projetos como BMI Calculator, Meetapp, Ecoleta e Be the Hero. TypeScript aparece no currículo, na iTOP, em Unicrow, em vários projetos e no conteúdo técnico.
...
- menu desktop e mobile, para esconder o link Ask;
...
## Por que não usei outro runtime na versão final?

Um framework agentic entrou na conversa como hipótese para experimentar agentes e workflows locais. No fim, essa camada não ficou na arquitetura porque a feature pública não precisava de outro runtime.
...
O fix de layout foi encurtar a resposta fallback e mover trechos completos para os cards de fontes. O fix de corpus foi usar apenas `plainBody` seguro e adicionar documentos públicos de perfil, CV e projetos. O fix de busca foi combinar Fuse, score literal e stop words. O fix de rollout foi centralizar `NEXT_PUBLIC_FF_ASK_ENABLED`.
...
- evitar um framework agentic quando a feature não precisa dele em produção.

## Resumo

O Ask Thiago começou como uma ideia de hype bem recortada: um RAG público sobre o próprio site. A implementação virou uma feature pequena, com rota bilíngue, API controlada, corpus público, fallback sem LLM, síntese opcional via Vercel AI Gateway e rollout por `NEXT_PUBLIC_FF_ASK_ENABLED`.
