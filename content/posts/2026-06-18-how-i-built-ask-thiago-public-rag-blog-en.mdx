...
The first question was where an agentic interface made sense around this project without turning the site into a loose lab. The scope was pragmatic: keep Next.js and Velite as the public runtime, use only published content, and build search with cited answers.
...
5. prefer documents in the current page language;
6. return up to six sources with excerpts.

This also fixed real questions such as "o que ele sabe sobre react?", "Thiago sabe React Native?", and "Does Thiago know TypeScript?". The answer needs to cross CV, projects, and posts. React Native appears in the CV, the SwitchCare experience, and projects such as BMI Calculator, Meetapp, Ecoleta, and Be the Hero. TypeScript appears in the CV, iTOP, Unicrow, many projects, and technical writing.
...
- desktop and mobile menu, to hide the Ask link;
...
## Why did I not use another runtime in the final version?

An agentic framework came up as an option for local agent and workflow experiments. In the end, that layer did not stay in the architecture because the public feature did not need another runtime.
...
- avoid an agentic framework when the production feature does not need one.

## TL;DR

Ask Thiago started as a small, useful hype bet: a public RAG over my own site. The implementation became a focused feature with a bilingual route, controlled API, public corpus, search fallback, optional Vercel AI Gateway synthesis, and rollout through `NEXT_PUBLIC_FF_ASK_ENABLED`.
