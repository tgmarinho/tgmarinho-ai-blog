...
```text
[Modelo / App de IA]  ←→  [MCP Client]  ←→  [MCP Server]  ←→  [Dados / Ferramenta]
       (Host)                                                   (Postgres, GitHub,
   Claude Desktop,                                                arquivos, Slack...)
   Claude Code,
   Cursor, ChatGPT
```
...
1. Usuário digita pergunta
   "quantas inscrições teve meu evento?"
              │
              ▼
2. CLIENT (Claude/Cursor/etc) envia para o LLM
   prompt + lista de tools disponíveis (vem do MCP server)
   ┌───────────────────────────────────────────┐
   │ tools = [ listar_inscricoes, criar_evento │
   │           gerar_link, reembolsar, ... ]   │
   └───────────────────────────────────────────┘
              │  (gasta tokens de input)
              ▼
3. LLM decide qual tool chamar
   → "chame listar_inscricoes(eventoId='abc')"
              │  (gasta tokens de output)
              ▼
4. CLIENT envia chamada JSON-RPC ao MCP SERVER
   ┌──────────────────────────────────────────┐
   │ {"method":"tools/call",                  │
   │  "params":{"name":"listar_inscricoes",   │
   │             "arguments":{"eventoId":...}}│
   └──────────────────────────────────────────┘
              │  (0 tokens — é só JSON pela rede)
              ▼
5. MCP SERVER executa a função
   → traduz para chamada normal: tRPC / SQL / API interna
   → autentica + autoriza (OAuth, RBAC)
   → retorna { total: 22 }
              │  (0 tokens — é só JSON)
              ▼
6. CLIENT manda o resultado de volta pro LLM
   "tool result: { total: 22 }"
              │  (gasta tokens de input — sim, o retorno volta como input!)
              ▼
7. LLM redige a resposta final
   "Seu evento teve 22 inscrições pagas até agora 🎉"
              │  (gasta tokens de output)
              ▼
8. Resposta entregue ao usuário
```
...
```text
A ferramenta já é famosa, com CLI estável (git, gh, docker, kubectl, psql, aws)?
        │
        ├── SIM → use CLI. O modelo já sabe a sintaxe.
        │         Sem overhead de schema.
        │
        └── NÃO → é ferramenta sua / proprietária?
                  │
                  ├── E você controla o client?
                  │   → SDK + function calling direto.
                  │
                  └── E o client é de terceiros (Claude, ChatGPT)?
                      → MCP server. É o único jeito de aparecer pra eles.
```
...
```text
                  ...
                  ┌──────────┴───────────────┐
                  │                          │
                 SIM                        NÃO → REST/tRPC normal
                  │
       ┌──────────┴──────────┐
       │                     │
   Você controla       Client é de
   client E server?    terceiros?
       │                     │
       │                     ▼
       ▼              ┌─────────────┐
 SDK + function       │ MCP server  │ ← OAuth, escopo,
 calling direto       │ + auth +    │   tools enxutas
 (zero overhead)      │ tools mín.  │
                      └─────────────┘
                            │
              ┌─────────────┴───────────────┐
              │                             │
     Ferramenta famosa             Ferramenta sua /
     (git, gh, kubectl, psql)?     proprietária?
              │                             │
              ▼                             ▼
        Use CLI também                Expor via MCP
        (modelo já sabe;              é o caminho
         sem overhead de schema)
```
